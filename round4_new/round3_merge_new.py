...
class Trader:
    ...
    def take_best_orders(
        ...
        prevent_adverse: bool = False,
        adverse_volume: int = 0,
        position_limit: Union[int, None] = None,
    ) -> tuple[int, int]:
    ...
    def predict_option_vol_curve(
        ...
        spot_price: float,
        vol_curve: npt.NDArray[float],
        traderData: Dict[str, Any],
        product: str = "VOLCANIC_ROCK",
        strikes: npt.NDArray[int] = STRIKES,
    ) -> npt.NDArray[float]:
    ...
    def predict_option_delta(
        ...
        tte: float,
        spot_price: float,
        predicted_vol_curve: npt.NDArray[float],
        strikes: npt.NDArray[int] = STRIKES
    ) -> npt.NDArray[float]:
    ...
    def option_orders(
        ...
        theo_base_iv: float,
        current_delta: float,
        option_positions: npt.NDArray[int],
        deltas: npt.NDArray[float],
        vegas: npt.NDArray[float],
        traderData: Dict[str, Any],
        vol_curve: npt.NDArray[float],
        product: str = "VOLCANIC_ROCK",
        strikes: npt.NDArray[int] = STRIKES,
        trade_mask: Union[None, npt.NDArray[bool]] = None,
    ...
    def make_MAGNIFICENT_MACARONS_orders(
            self,
            order_depth: OrderDepth,
            fair_value: float,
            min_edge: float,
    ...
    def MAGNIFICENT_MACARONS_fair_value(
            self,
            order_depth: OrderDepth,
    ...
    def MAGNIFICENT_MACARONS_adap_edge(
        ...
        if len(self.MAGNIFICENT_MACARONS_data["volume_history"]) < self.params[Product.MAGNIFICENT_MACARONS]["volume_avg_timestamp"]:
            ...
            if volume_avg >= self.params[Product.MAGNIFICENT_MACARONS]["volume_bar"]:
                self.MAGNIFICENT_MACARONS_data["volume_history"] = []  # clear volume history if edge changed
            ...
            elif self.params[Product.MAGNIFICENT_MACARONS]["dec_edge_discount"] * self.params[Product.MAGNIFICENT_MACARONS]["volume_bar"] * (
        ...
        self.MAGNIFICENT_MACARONS_data["curr_edge"] = curr_edge
        return curr_edge
    ...
    def MAGNIFICENT_MACARONS_arb_clear(
        ...
        if position >= 0:
            conversions = max(-position, -limit)
        else:
            conversions = min(-position, limit)
        return conversions
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        if Product.MAGNIFICENT_MACARONS in self.params and Product.MAGNIFICENT_MACARONS in state.order_depths:
            ...
            MAGNIFICENT_MACARONS_fair_value = self.MAGNIFICENT_MACARONS_fair_value(
            ...
            conversions = self.MAGNIFICENT_MACARONS_arb_clear(MAGNIFICENT_MACARONS_position,
                                                              self.params[Product.MAGNIFICENT_MACARONS]["conversion_limit"])
            ...
            #
            # )
            # MAGNIFICENT_MACARONS_make_orders, _, _ = self.make_MAGNIFICENT_MACARONS_orders(
            #     state.order_depths[Product.MAGNIFICENT_MACARONS],
            #     MAGNIFICENT_MACARONS_fair_value,
