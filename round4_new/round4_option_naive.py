...
class BlackScholes:
    ...
    @staticmethod
    def implied_volatility(
        call_price, spot, strike, time_to_maturity, max_iterations=100, tolerance=1e-5, learning_rate=1
    ):
...
class Trader:
    ...
    def take_best_orders_ink(
        ...
        std = math.sqrt(traderData["price_square_mean"] - traderData["price_mean"] ** 2)
        ...
        if len(order_depth.sell_orders) != 0:
            ...
            if not prevent_adverse or abs(best_ask_amount) <= adverse_volume:
                quantity = 0
                best_ask_zscore = (best_ask - mean) / std 
        ...
        if len(order_depth.buy_orders) != 0:
            ...
            best_bid_zscore = (best_bid - mean) / std
    ...
    def spread_orders(
        ...
        spread_std = math.sqrt(spread_data["spread_square_mean"] - spread_data["spread_mean"] ** 2) if "default_spread_std" not in self.params[spread_product] else self.params[spread_product]["default_spread_std"]
    ...
    def get_ask_vol_curve(
        ...
        for i, strike in enumerate(strikes):
            option_order_depth = order_depths[f"{product}_VOUCHER_{strike}"]
            option_price = min(option_order_depth.sell_orders.keys()) if len(option_order_depth.sell_orders) > 0 else np.nan
            if not np.isnan(option_price):
                vol_curve[i] = BlackScholes.implied_volatility(
                    option_price,
                    spot_price,
                    strike,
    ...
    def get_bid_vol_curve(
        ...
        for i, strike in enumerate(strikes):
            option_order_depth = order_depths[f"{product}_VOUCHER_{strike}"]
            option_price = max(option_order_depth.buy_orders.keys()) if len(option_order_depth.buy_orders) > 0 else np.nan
            if not np.isnan(option_price):
                vol_curve[i] = BlackScholes.implied_volatility(
                    option_price,
                    spot_price,
    ...
    def predict_option_greeks(
        ...
        for i, strike in enumerate(strikes):
            ...
            d1 = BlackScholes.black_scholes_d1(
                spot_price,
                strike,
                tte - 1 / 2500000,
                vol,
            )
            predicted_deltas[i] = NormalDist().cdf(d1)
            predicted_vegas[i] = NormalDist().pdf(d1) * (spot_price * math.sqrt(tte))
        return predicted_deltas, predicted_vegas
    ...
    def short_option_order(
        ...
        target_option_position = round(-self.LIMIT[f"{product}_VOUCHER_{strike}"] * option_target_position_ratio)
    ...
    def long_option_order(
        ...
        target_option_position = round(self.LIMIT[f"{product}_VOUCHER_{strike}"] * target_option_position_ratio)
        if option_position >= target_option_position:
            return [], []
    ...
    def option_orders(
        ...
        product: str = "VOLCANIC_ROCK",
        strikes: List[int] = STRIKES,
        trade_mask: None | List[bool] = None,
    ) -> tuple[List[List[Order]], List[List[Order]], float]:
        if trade_mask is not None:
            assert(len(trade_mask) == len(strikes))
        else:
            trade_mask = [True] * len(strikes)
        take_order = [[]] * len(strikes)
        make_order = [[]] * len(strikes)
        for i, strike in enumerate(strikes):
            if np.isnan(vol_curve[i]) or not trade_mask[i]:
                continue
            traderData = traderObject[f"{product}_VOUCHER_{strike}"]
            ...
            if len(traderData["option_iv_history"]) <= self.params[product]["option_iv_window"]:
                ...
                continue
            std = math.sqrt(traderData["option_iv_square_mean"] - traderData["option_iv_mean"] ** 2)
            ...
            if not np.isnan(bid_vol_curve[i]) and (bid_vol_curve[i] - fair_vol) * vegas[i] > self.params[product]["option_price_threshold"]:
                option_take_order, option_make_order = self.short_option_order(
                    strike,
                    order_depths[f"{product}_VOUCHER_{strike}"],
                    option_positions[i],
                    (bid_vol_curve[i] - fair_vol) / std / self.params[product]["option_upper_vol_threshold"],
                    product,
                )
            ...
            elif not np.isnan(ask_vol_curve[i]) and (fair_vol - ask_vol_curve[i]) * vegas[i] > self.params[product]["option_price_threshold"]: # and (ask_vol_curve[i] - fair_vol) / std < -self.params[product]["lower_vol_threshold"]:
                option_take_order, option_make_order = self.long_option_order(
                    strike,
                    order_depths[f"{product}_VOUCHER_{strike}"],
                    option_positions[i],
                    (fair_vol - ask_vol_curve[i]) / std / self.params[product]["option_lower_vol_threshold"],
                    product,
                )
    ...
    #         fair_vol_curve = x * parabolic_coefprint(f"Current Vol: {vol_curve[i]} Fair Vol: {fai + theo_base_iv
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        if product in self.params and product in state.order_depths and len(state.order_depths[product].buy_orders) > 0 and len(state.order_depths[product].sell_orders) > 0:
            ...
            predicted_delta, predicted_vega = self.predict_option_greeks(tte, spot_price, vol_curve, traderObject)
            ...
            take_orders, make_orders, current_delta = self.option_orders(
                ...
                product,
                STRIKES,
                trade_mask = np.array([False, False, False, True, False])
            )
        ...
        if state.timestamp % 100000 == 0:
            print(f"Time: {state.timestamp}")
        #     print(f"Position: {state.position.get('VOLCANIC_ROCK_VOUCHER_100000', 0)}")
