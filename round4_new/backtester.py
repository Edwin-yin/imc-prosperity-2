...
class Backtester:
    def __init__(self, trader, listings: Dict[str, Listing], position_limit: Dict[str, int], fair_marks,
                 ...
                 observation_data: Union[None, pd.DataFrame] = None,
                 file_name: str = None, only_use_log: bool = False,
            ...
            if self.output_fair:
                for product in self.listings.keys():
                    if self.pnl[product] != 0:
                        product_df = copy.deepcopy(
                            self.market_data[self.market_data['product'] == product].reset_index(drop=True))
                ...
                for product in products:
                    ...
                    if product == "MAGNIFICENT_MACARONS":
                        self._execute_conversion(conversions, order_depths_matching, self.current_position, self.cash,
                                                 now_observation.conversionObservations["MAGNIFICENT_MACARONS"])

                    for order in orders.get(product, []):
                        executed_orders = self._execute_order(timestamp, order, order_depths_matching,
                                                              self.current_position, self.cash, bot_trade_history_dict,
                                                              sandboxLog)
    ...
    def _execute_conversion(self, conversions, order_depths, position, cash, observation):
        ...
        implied_ask = observation.askPrice + observation.transportFees - observation.importTariff + 0.1
        #implied_bid = observation.implied_bid
        #implied_ask = observation.implied_ask
        if conversions > 0:
            position["MAGNIFICENT_MACARONS"] += abs(conversions)
            cash["MAGNIFICENT_MACARONS"] -= implied_ask * abs(conversions)
        else:
            position["MAGNIFICENT_MACARONS"] -= abs(conversions)
            cash["MAGNIFICENT_MACARONS"] += implied_bid * abs(conversions)
...
if __name__ == '__main__':
    ...
    listings = {
        ...
        'MAGNIFICENT_MACARONS': Listing(symbol='MAGNIFICENT_MACARONS', product='PICNIC_BASKET2',
    ...
    for day in [1, 2, 3]:
        ...
        trade_history = pd.read_csv(f"./round-4-island-data-bottle/trades_round_4_day_{day}.csv", sep=";", header=0)
        market_observe = pd.read_csv(f"./round-4-island-data-bottle/observations_round_4_day_{day}.csv", sep=",",
                                     header=0)
        # market_data = market_data.merge(market_observe, on=['timestamp'], how='left')
        import io
