...
PARAMS = {
    ...
    Product.VOLCANIC_ROCK:{
        ...
        "default_parabolic_coef": 0.06, # TODO: use last one
        "option_iv_ewma_alpha": 0.05,
        "option_iv_window": 20,
        "option_base_iv_beta": -0.5,
        "option_parabolic_coef_beta": -0.491,
        "option_price_threshold": 1,
        "option_vega_threshold": 1.5, # in unit of 1%. Vega lower than this threshold will be onlhy used in hedging.
        "option_hedge_delta_threshold": 0.8,
        "option_take_width": 3,
        "option_clear_width": 1,
        "option_clear_threshold": 30,
        "option_disregard_edge": 0,
        "option_join_edge": 1,
        "option_default_edge": 2,
    },
...
class Trader:
    ...
    def MAGNIFICENT_MACARONS_take(
        ...
        sunlight_history = traderData.get("sunlight_history", [])
        now_sunlight = observation.sunlightIndex
        orders: List[Order] = []
        ...
        if ask > implied_ask:
            edge = (ask - implied_ask) * self.params[Product.MAGNIFICENT_MACARONS][
                "make_probability"
            ]
        else:
            edge = 0
        sunlight_history.append(now_sunlight)
        if len(sunlight_history) < self.params[Product.MAGNIFICENT_MACARONS]["sunlight_history_length"]:
            pass
        elif len(sunlight_history) > self.params[Product.MAGNIFICENT_MACARONS]["sunlight_history_length"]:
            sunlight_history.pop(0)

        now_condition = (now_sunlight <= self.params[Product.MAGNIFICENT_MACARONS]["csi"]) & (sunlight_history[0] >= now_sunlight)
        traderData["sunlight_history"] = sunlight_history
        ...
        for price in sorted(list(order_depth.buy_orders.keys()), reverse=True):
            if price < implied_ask + edge:
                if position > 0 and not now_condition:
                    quantity = min(
                        abs(order_depth.buy_orders[price]), sell_quantity, position
                    )  # max amount to sell
                    if quantity > 0:
                        orders.append(Order(Product.MAGNIFICENT_MACARONS, round(price), -quantity))
            ...
            if price > implied_ask + edge:
                ...
                if quantity > 0:
                    orders.append(Order(Product.MAGNIFICENT_MACARONS, round(price), -quantity))
                    sell_order_volume += quantity
        traderData["prev_condition"] = now_condition
    ...
    def hedge(
        ...
        if target_spot_quantity > 0:
            ...
            if len(spot_order_depth.sell_orders) > 0:
                ...
                quantity = min(
                    target_spot_quantity,
                    self.LIMIT[product] - spot_position,
                    -spot_order_depth.sell_orders[best_ask]
                )
                if quantity > 0:
                    orders_dict[product] = [Order(product, best_ask, round(quantity))]
                    target_spot_quantity -= round(quantity)
                    spot_order_depth.sell_orders[best_ask] += round(quantity)
                    spot_position += round(quantity)
                    if spot_order_depth.sell_orders[best_ask] == 0:
                        del spot_order_depth.sell_orders[best_ask]
            ...
            if len(spot_order_depth.buy_orders) > 0:
                ...
                quantity = min(
                    -target_spot_quantity,
                    self.LIMIT[product] + spot_position,
                    spot_order_depth.buy_orders[best_bid],
                )
                if quantity > 0:
                    orders_dict[product] = [Order(product, best_bid, -round(quantity))]
                    target_spot_quantity += round(quantity)
                    spot_order_depth.buy_orders[best_bid] -= round(quantity)
                    spot_position -= round(quantity)
                    if spot_order_depth.buy_orders[best_bid] == 0:
                        del spot_order_depth.buy_orders[best_bid]
        ...
        #         print(f"No available option to hedge at delta {target_spot_position}.")
        #     else:
        #         for i, option_delta in enumerate(sorted_deltas):
        #             strike = strikes[strike_index[i]]
        #             if target_spot_quantity < 1 or option_delta < self.params[product]["option_hedge_delta_threshold"]:
        #                 break
        ...
        #                     target_spot_quantity -= round(quantity) * option_delta
        #                     option_order_depth.sell_orders[best_ask] += round(quantity)
        #                     if option_order_depth.sell_orders[best_ask] == 0:
        #                         del option_order_depth.sell_orders[best_ask]
        ...
        #                     option_order_depth.buy_orders[best_bid] -= round(quantity)
        #                     if option_order_depth.buy_orders[best_bid] == 0:
        #                         del option_order_depth.buy_orders[best_bid]
        #         if target_spot_quantity <= -1:
        #             print(f"Cannot fully hedge delta with additional delta {target_spot_quantity}.")
                    
        if abs(target_spot_quantity) > 1:
            # print(f"Try using passive order to hedge delta {target_spot_quantity}.")
            if target_spot_quantity > 1:
                if len(spot_order_depth.buy_orders) > 0:
                    best_ask = max(spot_order_depth.buy_orders.keys()) + 1
                elif len(spot_order_depth.sell_orders) > 0:
                    best_ask = min(spot_order_depth.sell_orders.keys()) - 1
                else:
                    # print(f"Cannot hedge with passive order with addtional delta {target_spot_quantity}")
                    return orders_dict
                quantity = min(
                    target_spot_quantity,
                    self.LIMIT[product] - spot_position,
                ...
                if quantity > 0:
                    if product not in orders_dict:
                        orders_dict[product] = []
                    orders_dict[product].append(Order(product, best_ask, round(quantity)))
                    target_spot_quantity -= round(quantity)
            else:
                if len(spot_order_depth.sell_orders) > 0:
                    best_bid = min(spot_order_depth.sell_orders.keys()) - 1
                elif len(spot_order_depth.buy_orders) > 0:
                    best_bid = max(spot_order_depth.buy_orders.keys()) + 1
                else:
                    print(f"Cannot hedge with passive order with addtional delta {target_spot_quantity}")
                    return orders_dict
                quantity = min(
                ...
                if quantity > 0:
                    if product not in orders_dict:
                        orders_dict[product] = []
                    orders_dict[product].append(Order(product, best_bid, -round(quantity)))
                    target_spot_quantity += round(quantity)
    ...
    def option_orders(
        ...
        tte: float,
        spot_price: float,
        fair_vol_curve: np.ndarray[float] | None,
        current_delta: float,
        ...
        result = dict()
        trading_mask = np.ones(dtype=bool, shape=len(strikes))
        for i, strike in enumerate(strikes):
            if np.isnan(vol_curve[i]):
                continue
            option = f"{product}_VOUCHER_{strike}"
            traderData = traderObject[option]
            if fair_vol_curve is not None:
                fair_vol = fair_vol_curve[i]
            else:
                ...
                if vegas[i] < self.params[product]["option_vega_threshold"] or std * vegas[i] * 100 < self.params[product]["option_price_threshold"]:
                    trading_mask[i] = False
                    continue
            ...
            # Making strategy
            # option_take_orders, option_buy_volume, option_sell_volume = self.take_orders(
            #     option,
            #     order_depths[option],
            #     fair_price,
            #     self.params[product]["option_take_width"],
            #     int(option_positions[i]),                
            # )
            option_take_orders, option_buy_volume, option_sell_volume = [], 0, 0
            option_clear_orders, option_buy_volume, option_sell_volume = self.clear_orders(
                option,
                order_depths[option],
                fair_price,
                self.params[product]["option_clear_width"],
                int(option_positions[i]),
                option_buy_volume,
                option_sell_volume,
                self.params[product]["option_clear_threshold"]
            )
            # Calculate delta after taking
            current_delta += (option_buy_volume - option_sell_volume) * deltas[i]
            option_make_orders, option_buy_volume, option_sell_volume = self.make_orders(
                option, 
                order_depths[option],
                fair_price,
                int(option_positions[i]),
                option_buy_volume,
                option_sell_volume,
                self.params[product]["option_disregard_edge"],
                self.params[product]["option_join_edge"],
                self.params[product]["option_default_edge"],
                False,
                0
            )
            
            result[option] = option_take_orders + option_clear_orders + option_make_orders
            # # Taking strategy
            ...
            #     #     break
            #     # current_delta -= sum(order.quantity for order in option_make_order) * deltas[i]
        return result, current_delta, trading_mask
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        if Product.SQUID_INK in self.params and Product.SQUID_INK in state.order_depths:
            if Product.SQUID_INK not in traderObject.keys():
                ...
                #     # "price_mean": 0,
                #     # "price_square_mean": 0,
                #     "price_ewma": self.params[Product.SQUID_INK]["default_price_mean"],
                #     "price_square_ewma": self.params[Product.SQUID_INK]["default_price_std"] ** 2 + self.params[Product.SQUID_INK]["default_price_mean"] ** 2,
                # }
                traderObject[Product.SQUID_INK] = {
                    "high_price": None,
                    "low_price": None,
        ...
        if product in self.params and product in state.order_depths and len(state.order_depths[product].buy_orders) > 0 and len(state.order_depths[product].sell_orders) > 0:
            if product not in traderObject:
                traderObject[product] = {
                    "option_base_iv": self.params[product]["default_base_iv"],
            ...
            # theoretical_base_iv, theoretical_parabolic_coef = self.fair_iv(product, traderObject[product], state.order_depths, product)
            ...
            option_orders_dict, current_delta, trading_mask = self.option_orders(
                state.order_depths,
                tte,
                spot_price,
                None,  # TODO: calculate theoretical fair vol. Now using ewma
                current_delta,
                option_positions,
                predicted_delta,
                predicted_vega,
            ...
            hedge_orders_dict = self.hedge(
                ...
                state.position,
                current_delta,
                predicted_delta[~trading_mask],
                product,
                [strike for i, strike in enumerate(STRIKES) if not trading_mask[i]],
            )
        ...
        if Product.MAGNIFICENT_MACARONS in self.params and Product.MAGNIFICENT_MACARONS in state.order_depths:
            ...
            if Product.MAGNIFICENT_MACARONS not in traderObject.keys():
            ...
            MAGNIFICENT_MACARONS_take_orders, buy_order_volume, sell_order_volume = (
                self.MAGNIFICENT_MACARONS_take(
            ...
            MAGNIFICENT_MACARONS_make_orders = []
