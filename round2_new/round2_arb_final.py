...
class Trader:
    ...
    def spread_orders(
        ...
        if self.params[spread_product]["default_spread_mean"] is None:
            use_mean = np.mean(spread_data["spread_history"])
        else:
            use_mean = self.params[spread_product]["default_spread_mean"]
        if (basket_best_bid - synthetic_best_ask - use_mean) / spread_std >= self.params[spread_product]["zscore_threshold"]and basket_position > -self.params[spread_product]["target_position"]:
        ...
        if (basket_best_ask - synthetic_best_bid - use_mean) / spread_std <= -self.params[spread_product]["zscore_threshold"] and basket_position < self.params[spread_product]["target_position"]:
            return self.execute_spread_orders(
                self.params[spread_product]["target_position"],
                basket_position,
                basket_order_depth,
