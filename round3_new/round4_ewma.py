...
PARAMS = {
    ...
    Product.SQUID_INK: {
        ...
        "default_price_mean": 1933.5,
        "default_price_std": 21.63,
        "price_ewma_alpha": 0.001,
...
class Trader:
    ...
    def take_best_orders_ink(
        ...
        traderData["price_ewma"] = traderData["price_ewma"] * (1 - self.params[product]["price_ewma_alpha"]) + ink_fair_price * self.params[product]["price_ewma_alpha"]
        traderData["price_square_ewma"] = traderData["price_square_ewma"] * (1 - self.params[product]["price_ewma_alpha"]) + ink_fair_price ** 2 * self.params[product]["price_ewma_alpha"]
        # Calculate mean and std
        mean = traderData["price_ewma"]
        std = max(5e-5, math.sqrt(traderData["price_square_ewma"] - traderData["price_ewma"] ** 2))
    ...
    def take_orders_ink(
        ...
        buy_order_volume = 0
        sell_order_volume = 0
        # We need add timestamp as we should do nothing in the end
    ...
    def take_best_orders_ewma(
        ...
        traderData["price_ewma"] = traderData["price_ewma"] * (1 - self.params[product]["price_ewma_alpha"]) + current_mid * self.params[product]["price_ewma_alpha"]
        traderData["price_square_ewma"] = traderData["price_square_ewma"] * (1 - self.params[product]["price_ewma_alpha"]) + current_mid ** 2 * self.params[product]["price_ewma_alpha"]
        traderData["price_history"].append(current_mid)
        if len(traderData["price_history"]) <= self.params[product]["price_std_window"]:
            return 0, 0
        traderData["price_history"].pop(0)
        # Calculate mean and std
        fair_value = traderData["price_ewma"]
        std = max(5e-5, math.sqrt(traderData["price_square_ewma"] - traderData["price_ewma"] ** 2))
        # Process sell orders (best ask)
        if len(order_depth.sell_orders) != 0:
            best_ask = min(order_depth.sell_orders.keys())
            ...
            # Check adverse condition for sell orders
            if not prevent_adverse or abs(best_ask_amount) <= adverse_volume:
                if best_ask <= fair_value - std * self.params[product]["price_zscore_threshold"]: 
                    quantity = min(
                        best_ask_amount, position_limit - position
                    )  # max amt to buy
                    if quantity > 0:
        ...
        if len(order_depth.buy_orders) != 0:
            ...
            # Check adverse condition for buy orders
            if not prevent_adverse or abs(best_bid_amount) <= adverse_volume:
                if best_bid >= fair_value + self.params[product]["price_zscore_threshold"] * std:
                    quantity = min(
                        best_bid_amount, position_limit + position
                    )  # should be the max we can sell
                    if quantity > 0:
    ...
    def take_orders_ewma(
        ...
        buy_order_volume, sell_order_volume = self.take_best_orders_ewma(
            product,
            current_mid,
            orders,
            order_depth,
    ...
    def option_orders(
        ...
        for i, strike in enumerate(strikes):
            ...
            if vol_curve[i] >= fair_vol - std * 5 and vol_curve[i] <= fair_vol + std * 5:
                traderData["option_iv_history"].append(vol_curve[i])
                traderData["option_iv_ewma"] = traderData["option_iv_ewma"] * (1 - self.params[product]["option_iv_ewma_alpha"]) + vol_curve[i] * self.params[product]["option_iv_ewma_alpha"]
                traderData["option_iv_square_ewma"] = traderData["option_iv_square_ewma"] * (1 - self.params[product]["option_iv_ewma_alpha"]) + vol_curve[i] ** 2 * self.params[product]["option_iv_ewma_alpha"]
                fair_vol = traderData["option_iv_ewma"]
                std = max(5e-5, math.sqrt(traderData["option_iv_square_ewma"] - traderData["option_iv_ewma"] ** 2))
            ...
            zscore = (vol_curve[i] - fair_vol) / std
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        if Product.KELP in self.params and Product.KELP in state.order_depths:
            ...
            if kelp_mmmid is not None:
                ...
                #     )
                # )
                kelp_take_orders, buy_order_volume, sell_order_volume = list(), 0, 0
